...
def detect_anomalies(df):
    ...
    for _, row in df.iterrows():
        anomaly = call_dos_api(row["packet_rate"], row["packet_length"], row["inter_arrival_time"])
        row["anomaly"] = anomaly
        row["anomaly_score"] = 1.0 if anomaly == 1 else 0.0
...
def show_dos_dashboard():
    ...
    with tabs[2]:
        ...
        if st.button("Predict Anomaly"):
            anomaly = call_dos_api(packet_rate, packet_length, inter_arrival_time)
            result = {
                ...
                "anomaly": anomaly,
                "anomaly_score": 1.0 if anomaly == 1 else 0.0
            }
    ...
    with tabs[4]:
        ...
        if not df_hist.empty:
            ...
            if chart_type == "Line Chart":
                fig = px.line(df_hist, x="timestamp", y="packet_rate", color="anomaly", title="Historical DoS Metrics Over Time")
            elif chart_type == "Bar Chart":
                fig = px.bar(df_hist, x="timestamp", y="packet_rate", color="anomaly", title="Packet Rate Over Time")
            elif chart_type == "Pie Chart":
                fig = px.pie(df_hist, names=df_hist["anomaly"].map({0: "Normal", 1: "Attack"}), title="Anomaly Distribution in Historical Data")
            elif chart_type == "Graph":
                fig = px.scatter(df_hist, x="timestamp", y="packet_rate", color="anomaly", title="Packet Rate Scatter Plot")
            elif chart_type == "Area Chart":
                fig = px.area(df_hist, x="timestamp", y="packet_rate", color="anomaly", title="Packet Rate Area Chart")
