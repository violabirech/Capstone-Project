...
from tabs.utils import get_historical
from tabs.models import detect_anomalies_dos
...
def render(dashboard, thresh, highlight_color):
    ...
    col1, col2 = st.columns(2)
    with col1:
        start_date = st.date_input("Start Date", value=datetime.now() - timedelta(days=7), key="dos_start")
    with col2:
        end_date = st.date_input("End Date", value=datetime.now(), key="dos_end")

    df = get_historical(start_date, end_date, "dos", "realtime")
    ...
    rows_per_page = 100
    total_pages = (len(df) - 1) // rows_per_page + 1
    page = st.number_input("Page", 1, total_pages, 1, key="dos_page") - 1
    df_page = df.iloc[page * rows_per_page:(page + 1) * rows_per_page]
    ...
    chart_type = st.selectbox("Chart Type", ["Line", "Bar", "Pie", "Area", "Scatter"], key="dos_chart")
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.download_button("Download CSV", df.to_csv(index=False), "dos_historical_data.csv")
